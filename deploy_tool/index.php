...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_GET['action'])) {
    ...
    if ($action === 'apply_domains') {
        ...
        sse_message("🌐 Applying domains to Nginx (no file deploy)...");
        try {
            [$ssh, $sudo] = ssh_connect($host, $port, $user, $password);
            $configKey = nginx_config_key($main_domain);
            ...
            if ($main_domain !== '') {
                ...
                if (trim($checkLe) === 'EXISTS') {
                    $useLetsEncrypt = true;
                    sse_message("ℹ️ Found Let's Encrypt certificates. Using them.");
                }
            }
            ...
            sse_message("📝 Writing Nginx config: $configKey");
            write_nginx($ssh, $sudo, $configKey, $nginxConfig);

            sse_message("✅ Domains applied.", 'success');
            sse_message("DONE_APPLY_DOMAINS");
            exit;
        } catch (Exception $e) {
            sse_message("❌ Apply domains error: " . $e->getMessage(), 'error');
            exit;
        }
    }
    ...
    if ($action === 'remove_domains_only') {
        ...
        sse_message("🧹 Removing managed domains from Nginx (keeping files)...");
        try {
            [$ssh, $sudo] = ssh_connect($host, $port, $user, $password);
            $configKey = nginx_config_key($main_domain);

            sse_message("🗑️ Removing Nginx config: $configKey");
            remove_nginx_config($ssh, $sudo, $configKey);

            sse_message("✅ Domains removed from Nginx.", 'success');
            sse_message("DONE_REMOVE_DOMAINS");
            exit;
        } catch (Exception $e) {
            sse_message("❌ Remove domains error: " . $e->getMessage(), 'error');
            exit;
        }
    }
    ...
    if ($action === 'update_page') {
        ...
        if (!is_dir($sourceDir)) {
             sse_message("❌ Local source directory does not exist: $sourceDir", 'error');
             exit;
        }

        sse_message("📦 Zipping page assets...");
        $zipFile = __DIR__ . '/deploy_package.zip';
        if (!zip_project_page($sourceDir, $zipFile)) {
            sse_message("❌ Failed to zip page assets.", 'error');
            exit;
        }
        ...
        try {
            ...
            sse_message("⬆️ Uploading files...");
            $ssh->exec("mkdir -p " . escapeshellarg($path));
            if (!$sftp->put($path . '/deploy_package.zip', $zipFile, SFTP::SOURCE_LOCAL_FILE)) {
                throw new Exception("Upload failed.");
            }
        } catch (Exception $e) {
            sse_message("❌ Connection/Upload Error: " . $e->getMessage(), 'error');
            unlink($zipFile);
            exit;
        }

        sse_message("⚙️ Applying page updates...");
        $commands = [
        ...
        unlink($zipFile);
        sse_message("✅ Page updated successfully.", 'success');
    ...
    if ($action === 'update_code') {
        ...
        sse_message("📦 Zipping project files...");
        $zipFile = __DIR__ . '/deploy_package.zip';
        if (!zip_project($sourceDir, $zipFile)) {
            sse_message("❌ Failed to zip project.", 'error');
            exit;
        }
        ...
        sse_message("⚙️ Applying updates...");
        $commands = [
            "cd " . escapeshellarg($path) . " && unzip -o deploy_package.zip",
            "rm " . escapeshellarg($path) . "/deploy_package.zip",
            "chown -R www-data:www-data " . escapeshellarg($path),
        ...
        try {
            ...
            if ($ssh->login($user, $password)) {
             ...
             sse_message("⚠️ Worker restart warning: " . $e->getMessage(), 'warning');
        }

        unlink($zipFile);
        sse_message("✅ Code updated successfully.", 'success');
    ...
    if ($action === 'deploy' || $action === 'update') {
        ...
        if (!$syntaxCheck['success']) {
            sse_message("❌ Syntax Error in " . $syntaxCheck['file'], 'error');
            sse_message($syntaxCheck['error'], 'error');
            sse_message("⚠️ Deployment Aborted due to syntax error.");
            exit;
        }
        ...
        if (zip_project($sourceDir, $zipFile)) {
            sse_message("✅ Project zipped successfully (" . round(filesize($zipFile) / 1024 / 1024, 2) . " MB).");
        } else {
            sse_message("❌ Failed to zip project.", 'error');
            exit;
        }
        ...
        try {
            ...
            sse_message("✅ SFTP Connected.");

        } catch (Exception $e) {
            sse_message("❌ Connection Error: " . $e->getMessage(), 'error');
            unlink($zipFile);
            exit;
        }

        // 3. Upload Zip
        sse_message("⬆️ Uploading project files...");
        try {
            ...
            // Upload
            if ($sftp->put($path . '/deploy_package.zip', $zipFile, SFTP::SOURCE_LOCAL_FILE)) {
                sse_message("✅ Upload complete.");
            } else {
                throw new Exception("Upload failed.");
            }
        } catch (Exception $e) {
            sse_message("❌ Upload Error: " . $e->getMessage(), 'error');
            unlink($zipFile);
            exit;
        }

        // 4. Remote Execution & Nginx Setup
        sse_message("⚙️ Configuring remote environment...");
        ...
        sse_message("🚀 Starting Worker Process...");
        ...
        try {
            ...
            if ($ssh->login($user, $password)) {
                ...
                sse_message("❌ Failed to reconnect for worker start", 'error');
            }
        } catch (Exception $e) {
            sse_message("⚠️ Worker start warning: " . $e->getMessage(), 'warning');
        }

        // 5. SSL Setup (Self-Signed for Cloudflare Full)
        sse_message("🔒 Checking SSL certificates...");
        ...
        try {
            $ssh = new SSH2($host, $port);
            if (!$ssh->login($user, $password)) {
                 sse_message("❌ Failed to reconnect for SSL setup", 'error');
            } else {
                 $sudo = ($user === 'root') ? '' : 'sudo ';
                 ...
                 if (trim($checkCert) !== 'EXISTS') {
                     sse_message("🔑 Generating self-signed certificate...");
                     ...
                     if (trim($checkAgain) === 'CREATED') {
                         sse_message("✅ Certificate generated.");
                     } else {
                         sse_message("⚠️ Certificate generation failed. Nginx SSL might fail.", 'warning');
                     }
                 } else {
                     sse_message("✅ Certificate already exists.");
                 }
            }
        } catch (Exception $e) {
            sse_message("⚠️ SSL Setup Warning: " . $e->getMessage(), 'warning');
        }
        ...
        try {
            ...
            sse_message("🌐 Configuring Nginx for: $targetLabel...");
            ...
            write_nginx($ssh, $sudo, $configKey, $nginxConfig);
            sse_message("✅ Nginx configured and reloaded.");

            // Post-Deploy Health Check
            sse_message("🏥 Performing Post-Deploy Health Check...");
            
            $checkPath = '/';
            if ($rotation_enabled && !empty($rotation_path) && !empty($rotation_slugs)) {
                $checkPath = '/' . trim($rotation_path, '/') . '/' . $rotation_slugs[0];
                sse_message("ℹ️ Checking rotation path: $checkPath");
            }
            ...
            if ($httpCode >= 200 && $httpCode < 400) {
                sse_message("✅ Health Check Passed: Localhost returned HTTP $httpCode");
            } else {
                sse_message("⚠️ Health Check Warning: Localhost returned HTTP $httpCode. Check Nginx logs.", 'warning');
            }

            if ($main_domain !== '') {
                $publicUrl = "http://$main_domain" . $checkPath;
                $codePublic = trim($ssh->exec("curl -s -o /dev/null -w '%{http_code}' " . escapeshellarg($publicUrl)));
                sse_message("ℹ️ Public URL Check ($publicUrl): HTTP $codePublic");
            }

        } catch (Exception $e) {
            sse_message("⚠️ Nginx Setup Warning: " . $e->getMessage(), 'warning');
        }
    ...
    if ($action === 'ssl') {
        sse_message("🔒 Starting SSL Configuration (Certbot)...");
        ...
        try {
            ...
            if ($wildcard_enabled) {
                sse_message("ℹ️ Wildcard enabled in Nginx. Certbot will certify listed domains only (HTTP-01).");
            }
            ...
            // DNS Pre-check
            sse_message("🔍 Verifying DNS records...");
            $vpsIp = gethostbyname($host); // Resolve VPS hostname to IP if needed
            foreach ($certDomains as $d) {
                if ($d === $host) continue;
                $domainIp = gethostbyname($d);
                if ($domainIp !== $vpsIp && $domainIp !== $host) {
                     sse_message("⚠️ DNS Warning: $d resolves to $domainIp, but VPS is $host. Certbot might fail.", 'warning');
                } else {
                     sse_message("✅ DNS Verified: $d -> $domainIp");
                }
            }
            ...
            sse_message("✅ SSL Configured Successfully!", 'success');
            sse_message("DONE_SSL");

        } catch (Exception $e) {
            sse_message("❌ SSL Error: " . $e->getMessage(), 'error');
            exit;
        }
        exit;
    }
    ...
    if ($action === 'delete_uninstall') {
        sse_message("⚠️ Starting cleanup sequence...");
        ...
        try {
            ...
            sse_message("🛑 Stopping worker and background processes...");
            ...
            // 3. Remove Files
            sse_message("🗑️ Removing project files at $path...");
            ...
            sse_message("DONE_CLEANUP");
             ...
             sse_message("❌ Cleanup Error: " . $e->getMessage(), 'error');
        }
        exit;
    }
}
                        ...
                        <div class="bg-slate-900/50 border border-white/10 rounded-xl p-6 backdrop-blur-sm shadow-xl hover:border-brand-500/50 transition cursor-pointer group relative" onclick="openDeploy('<?php echo $index; ?>')">
                            ...
                            <?php if(count($allDomains) > 0): ?>
                                <p class="text-xs text-slate-400">
                                    Domains: <?php echo htmlspecialchars(implode(', ', array_slice($allDomains, 0, 4))); ?><?php echo (count($allDomains) > 4) ? '…' : ''; ?>
                                </p>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
        ...
        <div id="view-add-server" class="hidden max-w-2xl mx-auto">
            ...
            <form id="addServerForm" class="bg-slate-900/50 border border-white/10 rounded-xl p-8 backdrop-blur-sm shadow-xl space-y-6">
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div class="col-span-2 relative input-group">
                        <input type="password" name="password" id="add_password" required class="block px-4 py-3 w-full text-sm text-white bg-black/20 rounded-lg border border-gray-700 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 focus:outline-none peer placeholder-transparent" placeholder="••••••••">
                ...
                <form id="deployForm" class="flex-1 flex flex-col bg-slate-900/50 border border-white/10 rounded-xl overflow-hidden backdrop-blur-sm shadow-xl">
                    ...
                    <div class="p-3 overflow-y-auto space-y-3 custom-scrollbar flex-1">
                        ...
                        <div class="border-t border-white/5 pt-2 mt-2 space-y-2">
                            <div class="flex items-center gap-2 text-[11px] font-semibold text-gray-300 mb-1">
